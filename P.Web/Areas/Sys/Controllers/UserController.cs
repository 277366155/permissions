...
namespace P.Web.Areas.Sys.Controllers
{
    public class UserController : Controller
    {
        ...
        public JsonResult GetUsers(int limit, int offset, string userName, int enable)
        {
            ...
            var lstUsers = users.OrderByDescending(t => t.CreateTime).Skip(offset).Take(limit).ToList();
        ...
        public ActionResult Create(UserVM userVm)
        {
            ...
            if (!ModelState.IsValid) return Json(new Result(ResultType.ParamError, "参数错误，请重新检查输入"));
        ...
        public ActionResult SetRoles(int keyId, string[] chkRoles)
        {
            if (keyId <= 0)
            {
                return Json(new Result(ResultType.ParamError, "参数错误!"));
            }
            Result result = _userService.UpdateUserRoles(keyId, chkRoles);
            result.Message = result.Message ?? result.ResultType.GetDescription();
            return Json(result);
        }
        #endregion
        ...
        public ActionResult SetUserGroups(int keyId, string[] chkUserGroups)
        {
            ...
            Result result = _userService.UpdateUserGroups(keyId, chkUserGroups);
            result.Message = result.Message ?? result.ResultType.GetDescription();
            return Json(result);
        }
        #endregion
